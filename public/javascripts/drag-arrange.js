...
(function (factory) {
  ...
  $.fn.arrangeable = function(options) {
    ...
    // ADDED: Counter was not resetted before.
    counter = 0;
  ...
  // ADDED
  function updateQueueListWithNewSwaps() {
    var updatedQueue = [];
    console.log("End drag");

    // TODO: Find if theres a better way to do this
    $(".media-card").each(function(index) {
      updatedQueue.push($(this).attr("id"));
    });

    socket.emit("From (drag-arrange): Update queue with swaps", updatedQueue);
  }
